...
//-------------------	VideoWriter out;
Mat edges;
My_ROI roi1, roi2,roi3,roi4,roi5;
vector <My_ROI> roi;
...
void average(MyImage *m){
	...
	for(int i =0 ;i<NSAMPLES; i++){
		cout << "avgColor[" << i << "][0] = " << avgColor[i][0] << ";" << endl;
		cout << "avgColor[" << i << "][1] = " << avgColor[i][1] << ";" << endl;
		cout << "avgColor[" << i << "][2] = " << avgColor[i][2] << ";" << endl;

		mediaColor0 += avgColor[i][0];
		mediaColor1 += avgColor[i][1];
		mediaColor2 += avgColor[i][2];
	}

	mediaColor0 /= NSAMPLES;
	mediaColor1 /= NSAMPLES;
	mediaColor2 /= NSAMPLES;

	cout << "0 :" << mediaColor0 << endl;
	cout << "1 :" << mediaColor1 << endl;
	cout << "2 :" << mediaColor2 << endl;
}
...
void myDrawContours(MyImage *m,HandGesture *hg){
	...
	if(hg->hullP[hg->hullP.size() - 1].size()!=0){
		for(int i=0;i<hg->hullP.size();i++){	
			for(int j=0;j<hg->hullP[i].size();j++){
				aux.x += hg->hullP[i][j].x;
				aux.y += hg->hullP[i][j].y;
				cont++;
			}
		}
		...
		circle(m->src, media, 10, cv::Scalar(0,255,0));
		...
		// if(cont2 < 10){
		// 	pontos[cont2].x = media.x;
		// 	pontos[cont2].y = media.y;
		// 	cont2++;
		// }else{
		// 	if(atualizar){
		// 		for(int i=1; i<9; i++){
		...
		// 	mediafinal.y =0;
		// 	for(int i=0; i<10; i++){
		// 		mediafinal.x += pontos[i].x;
		// 		mediafinal.y += pontos[i].y; 
		// 	}
		// 	mediafinal.x /= 10;
		// 	mediafinal.y /= 10;
		// 	atualizar = true;
		// 	circle(m->src, mediafinal, 10, cv::Scalar(0,255,0));
		// }
	}


	// cout << hg->hullP.size() << endl;
	// for(int i=0;i<hg->hullP.size();i++){
	// 	cout << hg->hullP[i].size() << endl;
	// }
	//imshow("testas",m->src);
